...
void pago_facturas() {
    int id_buscado,opcion,no_pagado = 0,total_pagar = 0,descuento = 0,centinela_activo = 0,id_contrato;
    system("cls");
    printf("Ingrese el id de la cuenta\n");
    scanf("%d",&id_buscado);
    listar_contratos_de_cliente(id_buscado);
    int contador=0;
    FILE *archivo;
    if((archivo=fopen("clientes/contratos.dat","rb"))!=NULL){
        contratos revision;
        fread(&revision,sizeof(revision),1,archivo);
        while(!feof(archivo)){
            if(revision.id==id_buscado && revision.estado_contrato==1){
                contador++;
            }
            fread(&revision,sizeof(revision),1,archivo);
        }
    ...
    if(contador==0){
        printf("El cliente no tiene contratos disponibles.\n");
    }else{
                printf("Ingresar el id del contrato deseado: ");scanf("%d",&id_contrato);
                system("cls");
                FILE *ap_facturas;
                ...
                if((ap_facturas = fopen("clientes/facturas.dat","rb")) != NULL) {
                    ...
                    while(!feof(ap_facturas)) {
                        ...
                        // printf("id_contrato = %d idFactura = %d\n",id_contrato,e_facturas.id_contrato);
                        if(id_contrato == e_facturas.id_contrato) {
                            ...
                            else
                            {
                                ...
                                if((ap_contratos = fopen("clientes/contratos.dat","r+b")) != NULL) {
                                    ...
                                    while(!feof(ap_contratos)) {
                                        ...
                                        if(e_contratos.id_contrato == id_contrato && estado_cliente == 1) {
                                        ...
                                        else
                                        {
                                            ...
                                            if(e_contratos.id == id_contrato && estado_cliente == 0) {
                                                centinela_activo = 0;
                                                no_pagado = 0;
                                            }
                                        }
                                ...
                                if(e_facturas.fecha_vencimiento.day < day) {
                                    ...
                                    printf("RECARGO POR VENCIMIENTO = $%.2f\n",total_pagar * 0.2);
                                    total_pagar += total_pagar * 0.2; //le hacemos un incremento del 20%
                                }
                            ...
                            if(id_contrato == e_facturas.id_contrato) {
                                ...
                                if(opcion == 1) {
                                    ...
                                    if((ap_contratos = fopen("clientes/contratos.dat","r+b")) != NULL) {
                                        ...
                                        while(!feof(ap_contratos)) {
                                            //buscamos al cliente
                                            if(e_contratos.id_contrato == id_contrato) {
